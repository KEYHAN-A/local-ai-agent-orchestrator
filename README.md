...
**LAO** (**v3.1.0+**) is a local coding factory for [LM Studio](https://lmstudio.ai/) and other **OpenAI-compatible** servers: a **planner → coder → verifier → reviewer** pipeline for long-running work, plus **Pilot Mode**—an interactive, agentic chat on your terminal that can run workspace tools, inspect the queue, create plans, switch projects, and hand control back to autopilot when you type **`/resume`**. Everything is backed by **SQLite**, **memory-aware model swapping**, optional **per-plan Git**, and a unified **TTY experience** (Rich + prompt_toolkit). **v3.1.0** adds structured **tool contracts**, **permissions**, a mechanical **verifier**, **skills**, **hierarchical memory**, **conversation compaction**, optional **MCP**, **`lao doctor`**, and more (see [What is new in v3.1.0](#what-is-new-in-v310) and [CHANGELOG.md](CHANGELOG.md)).
...
- [Features](#features)
- [What is new in v3.1.0](#what-is-new-in-v310)
...
Optional environment variables: **`LAO_VERSION`** (pin a release, e.g. `3.1.0`), **`LAO_PACKAGE`** (override PyPI name).
...
This release is a **reliability and quality** milestone: fewer “looks done but is not” outcomes, clearer operator control, and better fit with how serious local agent stacks are run today.

| Area | Highlights |
|------|------------|
| **Correctness** | **Mechanical verifier** between coder and reviewer (claimed files exist, Python/JSON/YAML parse, workspace path checks, **TODO ledger** completion). Coder retries without burning reviewer attempts when checks fail. |
| **Tools** | `tools/` **package** with a **`Tool` dataclass** registry: JSON-schema validation, auto-generated OpenAI tool list, centralized dispatch with **audit logging** (`tool_audit` table). |
| **Safety** | **`permissions`** modes (`auto`, `confirm`, `plan_only`, `bypass`) and wildcard rules (`Bash(…)`, `FileWrite(…)`, …). **`enter_plan_mode` / `exit_plan_mode`** block mutations until an explicit plan is approved. |
| **Guidance** | **Skills** (bundled + `.lao/skills/`), **hierarchical memory** (`LAO_MEMORY.md`, `~/.lao/MEMORY.md`), **conversation compaction** instead of naive tail-trimming. |
| **Interop** | Optional **MCP client** (`factory.yaml: mcp_servers`) and **`lao mcp-server`** (stdio) for other agents. **`lao doctor`** for grouped diagnostics. **Hooks** (`hooks.py`). Optional **OpenTelemetry** spans on tool calls. |
| **Determinism** | Per-role **`temperature`**, **`top_p`**, **`seed`**, **`repetition_penalty`** in YAML; **`lao run --seed`**, **`--permission-mode`**, **`--output-style`** (`terse` / `narrative` / `json`). |

Full detail: **[CHANGELOG.md](CHANGELOG.md)** → **v3.1.0**. Configuration keys: **[docs/CONFIGURATION.md](docs/CONFIGURATION.md)** (Reliability & Quality section).

### End-to-end pipeline

- **Planner (architect):** Decomposes a master plan into a JSON array of micro-tasks (title, description, file paths, dependencies). Large plans are **chunked** to fit the planner context; completed chunks are **resumed** instead of recomputed. When the model supports it, architect output can be constrained with **JSON schema** response mode (with regex fallback for older servers).
- **Task queue:** **SQLite** (WAL) stores plans, tasks, run logs, structured review findings, **per-task TODOs**, **memory facts**, and **tool audit** rows. **Dependency-aware** scheduling: tasks wait on prerequisites; dependents of **failed** tasks are failed with explicit feedback.
- **Coder:** OpenAI-style chat with **tool calling** (when the model supports it): `file_read`, `file_write`, `file_patch`, `list_dir`, `shell_exec`, `find_relevant_files`, **`task_todo_set` / `task_todo_get`**, **`enter_plan_mode` / `exit_plan_mode`**, **`memory_*`**, **`skill_list` / `skill_run`**, and more (see `tools/` package). Work runs inside the **active plan workspace** (see below).
- **Verifier (v3.1.0+):** Deterministic checks on coder output **before** the reviewer LLM runs; failures enqueue **rework** with structured hints.
- **Reviewer:** Single completion that must yield structured JSON (`verdict`, `findings`, `summary`). Feeds back into queue state (approve, rework, or fail after max attempts). Optional **JSON schema** mode when supported.
...
- When configured, **embedding search** (`find_relevant_files`) can rank files before coding so the coder prompt includes short excerpts from likely-relevant paths (implemented in `tools/search.py`).
...
Full release notes: **[CHANGELOG.md](CHANGELOG.md)** (latest: **`v3.1.0`**; Pilot highlights in `v3.0.4`).
...
lao doctor           # grouped diagnostics (LM Studio, models, git, disk, schema, …)
lao run              # watch plans/, run pipeline; enters Pilot when idle (TTY)
lao pilot            # jump straight into Pilot chat (after health check)
lao skills list      # bundled + user skills (Markdown workflows)
lao memory show      # project + user memory prelude (LAO_MEMORY.md, ~/.lao/MEMORY.md)
...
| **`git.*`** | Enable/disable traceability, plan snapshot filename, optional commit trailers; optional **`worktrees`** for speculative coder attempts. |
| **`models.*`** | Per-role **`key`**, **`context_length`**, **`max_completion`**, **`supports_tools`**, **`temperature`**, **`top_p`**, **`seed`**, **`repetition_penalty`**, **`supports_json_schema`**, size hints for memory accounting. Roles include **`pilot`**, **`analyst`**, **`embedder`**. |
| **`permissions.*`** | **`mode`** (`auto` / `confirm` / `plan_only` / `bypass`) and **`allow`** / **`deny`** wildcard rules for tools (see [docs/CONFIGURATION.md](docs/CONFIGURATION.md)). |
| **`verifier_enabled`**, **`compaction_*`** | Toggle mechanical verifier and conversation compaction (see [docs/CONFIGURATION.md](docs/CONFIGURATION.md)). |
| **`skills_*`**, **`memory_*`**, **`mcp_servers`**, **`output_style`**, **`otel.*`**, **`hooks.*`** | Skills, hierarchical memory, MCP servers, reply shape, telemetry, and user hooks (see [docs/CONFIGURATION.md](docs/CONFIGURATION.md)). |
...
| `lao doctor` | Grouped diagnostics: LM Studio, models, RAM budget, git, embedder, validators, disk, SQLite schema. |
| `lao mcp-server` | Run LAO as a minimal MCP server over stdio (expose curated tools to other agents). |
| `lao skills` | `list` (default) or `show <name>` — bundled and user-defined Markdown skills. |
| `lao memory` | `show` (default), `edit "<fact>"`, or `forget "<substring>"` — project and user memory files. |
...
| `--permission-mode` | `auto`, `confirm`, `plan_only`, or `bypass` (overrides `permissions.mode` for this run). |
| `--seed` | Override **`seed`** on every role model for this run (determinism experiments). |
| `--output-style` | `terse`, `narrative`, or `json` — shapes free-form system addenda for this run. |
...
flowchart LR
  ...
  coderLLM --> verifier[Mechanical_verifier]
  verifier --> reviewerLLM[Reviewer_LLM]
  reviewerLLM --> sqliteQ
...
**Recent highlights (v3.1.0):** **Verifier phase**, **structured tool registry**, **permissions**, **TODO ledger**, **skills**, **hierarchical memory**, **compaction**, **MCP** client/server, **`lao doctor`**, **hooks**, optional **OpenTelemetry**, per-role **determinism** knobs — see **[CHANGELOG.md](CHANGELOG.md)**. **v3.0.12** fixed analyst defaults in **`lao init`**. **v3.0.9** added validation inference, **`LAO_QUALITY.md`**, Pilot **`/gates`**. **LAO Pilot Mode** (v3.0.4+): interactive agentic chat, project registry (`lao projects`, `/project`), grouped home menu.
