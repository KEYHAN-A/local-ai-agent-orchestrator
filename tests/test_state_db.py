...
class TestTaskQueueStateHelpers(unittest.TestCase):
    ...
    def test_mark_rework_sets_retry_cooldown_and_attempt(self):
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            (root / ".lao").mkdir(parents=True, exist_ok=True)
            (root / "plans").mkdir(parents=True, exist_ok=True)
            ...
            t = q.get_plan_tasks(pid)[0]
            q.mark_rework(t.id, "needs fix")
            t2 = q.get_task(t.id)
            self.assertEqual(t2.attempt, 1)
            self.assertIsNotNone(t2.next_eligible_at)
            q.close()
